...
abstract public class AbstractHttpApi implements HttpApi {
    ...
    public HttpGet createHttpGet(String url, NameValuePair... nameValuePairs) {
        ...
        //HTTP淇℃伅鍙互鍖呭惈涓�郴鍒楀ご閮ㄦ弿杩版秷鎭殑灞炴�銆�        
    ...
    public static final DefaultHttpClient createHttpClient() {
        // Sets up the http part of the service. SchemeRegistry
        ...
        //self added ---璁剧疆HttpClient鏀寔HTTP鍜孒TTPS涓ょ妯″紡
       ...
       //self added ---
		...
		/*        
		 * ThreadSafeClientConnManager 鏄竴涓緢澶嶆潅鐨勫疄鐜帮紝瀹冪鐞嗕竴涓鎴疯繛鎺ユ睜锛屾湇鍔″涓墽琛岀嚎绋嬬殑杩炴帴璇锋眰锛岃繛鎺ヨ姣忎釜璺敱鏀惧叆姹犱腑銆�		 *  杩炴帴姹犱腑鍙敤鐨勫凡缁忓瓨鍦ㄦ寔涔呰繛鎺ョ殑璺敱璇锋眰鐢辨睜涓鍊熺殑杩炴帴杩涜鏈嶅姟锛岃�涓嶆槸鍒涘缓涓�釜鏂扮殑杩炴帴鍒嗘敮銆�		 * ThreadSafeClientConnManager鍦ㄦ瘡涓矾鐢变腑缁存寔涓�釜鏈�ぇ鐨勮繛鎺ラ檺鍒躲�缂虹渷鐨勫簲鐢ㄥ姣忎釜璺敱鍒涘缓浠呬粎2涓猚oncurrent杩炴帴锛屾�鏁颁笉瓒呰繃20涓�杩炴帴銆�		           瀵逛簬寰堝鐜板疄鐨勫簲鐢紝杩欎簺闄愬埗鍙兘鍑虹幇澶鐨勯檺鍒讹紝 鐗瑰埆鏄鏋滀粬浠娇鐢℉TTP浣滀负涓�釜 浼犺緭鍗忚杩涜鏈嶅姟銆傝繛鎺ラ檺鍒�鐒惰�锛屽彲浠ラ�杩嘓TTP鍙傛暟杩涜璋冩暣銆�		        // Increase max total connection to 200
    ...
    private static final HttpParams createHttpParams() {
    	//HttpParams鎺ュ彛浠ｈ〃涓�釜涓嶅彲鏀瑰彉鍊肩殑闆嗗悎锛屽畾涔変竴涓粍浠惰繍琛屾椂琛屼负銆�        
        ...
        //self added --- 璁剧疆涓�簺鍩烘湰鍙傛暟 HTTP璇锋眰鎵ц鍙傛暟
        HttpProtocolParamBean paramsBean = new HttpProtocolParamBean(params);
        paramsBean.setVersion(HttpVersion.HTTP_1_1);// 瀹氫箟浣跨敤HTTP鍗忚鐗堟湰, 濡傛灉涓嶈缃鍙傛暟灏嗕娇鐢℉TTP/1.1
        paramsBean.setContentCharset(HTTP.UTF_8);//瀹氫箟瀛楃闆嗙敤浜庢瘡涓唴瀹逛綋鐨勯粯璁ょ紪鐮�,濡傛灉涓嶈缃鍙傛暟灏嗕娇鐢↖SO-8859-1
        ConnManagerParams.setTimeout(params, 1000);   /* 浠庤繛鎺ユ睜涓彇杩炴帴鐨勮秴鏃舵椂闂�*/
        //self added ---
        
        //杩炴帴绠＄悊
        HttpConnectionParams.setConnectionTimeout(params, TIMEOUT * 1000);//寤虹珛杩炴帴鐨勮秴鏃舵椂闂达紙浠ユ绉掍负鍗曚綅锛夊綋鍊间负0琚В閲婃垚涓�釜鏃犻檺瓒呮椂,濡傛灉姝ゅ弬鏁颁笉璁剧疆锛岃繛鎺ユ搷浣滀笉浼氳秴鏃讹紙鏃犻檺瓒呮椂锛夈�
        HttpConnectionParams.setSoTimeout(params, TIMEOUT * 1000); //浠ユ绉掍负鍗曚綅瀹氫箟濂楁帴瀛楄秴鏃讹紙SO_TIMEOUT锛夈�褰撳�涓�琚В閲婃垚涓�釜鏃犻檺鐨勬殏鍋�濡傛灉姝ゅ弬鏁颁笉璁剧疆锛岃鎿嶄綔涓嶄細瓒呮椂锛堟棤闄愭殏鍋滐級銆�        
        HttpConnectionParams.setSocketBufferSize(params, 8192);//鎺ユ敹/浼犺緭HTTP娑堟伅鏃讹紝纭畾socket鍐呴儴缂撳啿鍖虹紦瀛樻暟鎹殑澶у皬,濡傛灉姝ゅ弬鏁颁笉璁剧疆锛孒ttpClient灏嗗垎閰�192瀛楄妭scoket缂撳啿鍖�
        return params;
    }
